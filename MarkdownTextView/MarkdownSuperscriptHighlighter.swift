    ...
    */
    ...
    public func highlight(attributedString: NSMutableAttributedString) {
        ...
        enumerateMatches(regex: type(of: self).SuperscriptRegex, string: attributedString.string) {
            ...
            attributedString.addAttributes(superscript(attributes: attributes, level: level, ratio: self.fontSizeRatio), range: textRange)
...
private func superscript(attributes: TextAttributes, level: Int, ratio: CGFloat) -> TextAttributes {
    if let font = attributes[.font] as? UIFont {
        let adjustedFont = UIFont(descriptor: font.fontDescriptor, size: font.pointSize * ratio)
        return [
            kCTSuperscriptAttributeName as NSAttributedString.Key: level,
            .font: adjustedFont
        ]
    }
    return [:]
